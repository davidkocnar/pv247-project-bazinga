  ...
  return (
      <Grid container spacing={2} >
        ...
        <Grid item container xs={12} lg={12} spacing={2}>
          {Object.keys(offers).filter(key => offers[key].categoryRef?.id.includes(selectedCategory)).filter(key => offers[key].title.toLowerCase().includes(search?.toLowerCase() || "")).map(key => {
